# AGENT.md — mgit 项目 AI 协作宪法

> 本文件是所有 AI Agent 在此工程中工作的最高行为准则。
> 版本：2026-03-17 · 工程版本：2.0.0-beta.6
...
**release-setup**：Tag 格式 `[0-9]*`（如 `2.0.0`、`2.0.0-beta.6`）
...
### 5.4 记忆更新义务（强制，每次任务完成后必须执行）

**完成任何任务后，必须立即执行以下更新，不需要用户提醒：**

#### `progress.md`
- 将完成的任务移入"已完成"列表，附上 commit hash
- 将进行中的任务推进到下一个状态
- 技术债表中将已修复项标记为 `已修复 {commit}`

#### `.ai/MEMORY.md`
- 将已修复的 bug 从"待处理"移入"已修复"区块，记录修复方案和 commit
- 将本次任务中验证的稳定结论（架构决策、踩坑经验）补充到对应主题下
- 删除或更正已过时的条目

#### `.ai/SNAPSHOT.md`
- 重大里程碑（版本发布、重要重构、架构变更）完成后追加记录
