...
///             └──2.txt
#[tokio::test]
async fn cli_clean1() -> MgitResult<()> {
    let tmp_dir = create_test_dir("test_clean1");
...
///     ├─... (same as cli_clean1())
#[tokio::test]
async fn cli_clean2() -> MgitResult<()> {
    let tmp_dir = create_test_dir("test_clean2");
    let path = tmp_dir.path().to_path_buf();
    let rel_paths = [
        ".",
        "foobar-1",
...
///     ├─... (same as cli_clean1())
#[tokio::test]
async fn cli_clean3() -> MgitResult<()> {
    let tmp_dir = create_test_dir("test_clean3");
    let path = tmp_dir.path().to_path_buf();
    let rel_paths = [
        "foobar-1",
        "foobar-1/foobar-1-1",
