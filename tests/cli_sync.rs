...
/// cmd: 'mgit sync ./target/tmp/test_sync_tracking_unvalid'
/// repos tree:
///   test_sync_tracking_unvalid(.git)
///     ├─foobar (.git)
///     └─1.txt
#[test]
fn cli_sync_tracking_unvalid() {
    let path = env::current_dir()
        .unwrap()
        .join("target/tmp/test_sync_tracking_unvalid");
    let input_path = path.to_str().unwrap();
...
"#;
    ...
    // set unvalid commit
    let toml_string = toml_string.replace("main", "unvalid-branch");
    // sync --hard will delete .gitrepos in the front
    std::fs::write(&config_file, toml_string.trim()).expect("Failed to write file .gitrepos!");
    // excute sync
    execute_cargo_cmd("mgit", &["sync", &input_path]);
